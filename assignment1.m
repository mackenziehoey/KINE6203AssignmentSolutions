...
%
% Answer: 'clear all' removes all the variables from the workspace 
...
%
% Answer: The function anova1 performs a one-way analysis of varience
% (ANOVA). 
...
%
% Answer: MATLAB uses for loops and while loops. A for loop will loops for
% a specified number of times. A while loops will continue to loop until a
...
%
% Answer: NaN stands for 'Not a Number'. This will be an answer when there
% is a not real or complex number as a result. For example, 0/0 will return
% NaN.
%
...
randVec = randi([-10,10],1,5);
sub3 = randVec - 3;
positiveVec = randVec(randVec >= 0);
absVal = abs(randVec);
maxVec = max(randVec);
...
base1 = 4; %Length of base 1 of trapezoid
base2 = 6; %Length of base 2 of trapezoid
height = 7; %Height of trapezoid
trapArea = height*0.5*(base1 + base2); %Area of trapezoid
...
vecX = input('Insert x value: ');
vecY = input('Insert y value: ');
vecZ = input('Insert z value: ');
magnitude = sqrt(vecX^2 + vecY^2 + vecZ^2);
vecX = vecX ./ magnitude;
vecY = vecY ./ magnitude;
vecZ = vecZ ./ magnitude;
unitVector = [vecX; vecY; vecZ];
fprintf('Your unit vector is <%.2f, %.2f, %.2f>\n', vecX, vecY, vecZ);
...
txt = input('Enter secret password: ',"s");
disp(txt)
